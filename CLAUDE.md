...
`config.Config` fields: `AutoBanEnabled`, `AutoBanThreshold`, `AutoBanWindow`, `AutoBanBanTime`, `ExtraIgnoreIPs`, `F2BMaxRetry`, `F2BBanTime`, `F2BFindTime` (int64 seconds; 0 = not managed). Unset `f2b.*` keys mean "never touch fail2ban" — `runF2BEnforce` exits its apply pass without any socket I/O.
...
1. Create `internal/store/migrations/NNN_description.sql` (e.g. `007_add_column.sql`; current latest is `006_ban_expiry.sql`).
