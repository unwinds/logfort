...
api.Server  (stdlib ServeMux, Go ≥1.22 method+path patterns)
    ...
    │                                 env-pinned notify fields are never written to the DB
...
`config.Config` fields: `AutoBanEnabled`, `AutoBanThreshold`, `AutoBanWindow`, `F2BMaxRetry`, `F2BBanTime`, `F2BFindTime` (int64 seconds; 0 = not managed). Unset `f2b.*` keys mean "never touch fail2ban" — `runF2BEnforce` exits its apply pass without any socket I/O.
