...
api.Server  (stdlib ServeMux, Go ≥1.22 method+path patterns)
    ...
    ├─ GET  /api/settings          → current notify config (from in-memory cfg)
    ├─ POST /api/settings          → persist to settings table + rebuild Dispatcher live
