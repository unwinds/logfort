...
LogFort reads your server's authentication log, parses every login attempt, and serves a live dashboard with:
...
docker run -d \
  ...
  -v $(pwd)/data:/data \
  -e LOGFORT_LOG_PATHS=/host/auth.log \
  ghcr.io/unwinds/logfort:latest
```

### Access over SSH tunnel

LogFort binds to `127.0.0.1` by default. To reach it from your machine:
...
- LogFort **listens on `127.0.0.1` by default**. Do **not** expose it directly to the internet without authentication (v1.0).
...
- The active ban feature (`LOGFORT_RESPONDER_ENABLED=true`) modifies your firewall. Enable it only intentionally.
...
|---|---|---|
| `LOGFORT_LISTEN` | `127.0.0.1:8080` | Bind address. **Keep localhost.** |
| `LOGFORT_BACKEND` | `file` | Log backend: `file` or `journald` |
| `LOGFORT_LOG_PATHS` | `/host/auth.log` | Comma-separated log file paths |
| `LOGFORT_FAIL2BAN_LOG` | *(empty)* | Path to fail2ban.log (optional) |
| `LOGFORT_DB_PATH` | `/data/logfort.db` | SQLite database file |
| `LOGFORT_GEOIP_DB` | `/data/geo.mmdb` | GeoIP mmdb file (optional, see below) |
| `LOGFORT_RETENTION_DAYS` | `90` | Delete events older than N days |
| `LOGFORT_HOME_LAT` / `_LON` | *(empty)* | Your server location for arc rendering |
| `LOGFORT_RESPONDER_ENABLED` | `false` | Enable active banning (opt-in) |
...
LogFort uses the [DB-IP Lite](https://db-ip.com/db/lite.php.html) (free, CC-BY) or MaxMind GeoLite2 format.
...
If no database is found, LogFort works without geo data (map shows "unknown").

---

## Comparison

| | LogFort | Grafana + Prometheus + Loki |
|---|---|---|
| Containers | **1** | 4–6 |
