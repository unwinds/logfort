...
func TestParseSSHDLines(t *testing.T) {
	tests := []struct {
		...
		{
			name:      "failed keyboard-interactive (PAM prompt)",
			line:      "Jun 21 14:32:04 myhost sshd[12346]: Failed keyboard-interactive/pam for invalid user admin from 198.51.100.8 port 41235 ssh2",
			wantType:  "failed_password",
			wantIP:    "198.51.100.8",
			wantUser:  "admin",
			wantPort:  41235,
		...
		},
		{
			...
			line:      "Jun 21 14:32:06 myhost sshd[12346]: Accepted keyboard-interactive/pam for carol from 192.0.2.33 port 41236 ssh2",
			wantType:  "accepted",
			wantIP:    "192.0.2.33",
			wantUser:  "carol",
			wantPort:  41236,
			wantValid: boolPtr(true),
			wantAuth:  "keyboard-interactive/pam",
		},
		{
			name:     "invalid_user",
